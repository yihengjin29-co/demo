import type { DemoState, LogEntry, Attachment, Role, WorkflowNodeRecord, IndicatorPeriodRecord, WarningLetterSetting } from '../types';
import { seedInstitutions } from './institutionData';
import { seedMajorRiskEventDefinitions } from './majorEventDefinitionService';
...
): WorkflowNodeRecord => ({ id, nodeId, nodeName, department: role === '各金融机构' ? '机构风险管理部门' : role === '集团' ? '集团管理部门' : '金控风险管理部', role, handler: role === '各金融机构' ? '机构经办人' : role === '集团' ? '集团经办人' : '张三', action, result, submittedAt, formData, attachments, status: result.includes('退回') ? '已退回' : '已完成', iteration: 1 });
...
export const normalizeState = (source: DemoState): DemoState => {
  ...
  state.warningDisposals.forEach(x => {
    ...
    if (x.letterDeliveryMode === 'auto') {
      x.letterSentBy ||= '系统自动下发';
      x.letterSentAt ||= x.triggerDate;
    }
    if (!x.workflow) {
      const records: WorkflowNodeRecord[] = [];
      if (x.reason) records.push(workflowRecord(`wf-${x.id}-1`, 'reason-fill', '原因分析填写', '各金融机构', { 原因分析: x.reason, 影响情况: '已纳入风险影响评估', 初步应对措施: x.plan || '已启动初步处置' }, '提交原因分析', '已提交', x.attachments, x.logs[0]?.time));
      if (x.reason) records.push(workflowRecord(`wf-${x.id}-2`, 'reason-review', '原因分析审核', '金控公司', { 审核意见: x.assessment || '原因分析完整，同意进入方案制定' }, '审核通过', '通过', [], x.logs[0]?.time));
      if (x.plan) records.push(workflowRecord(`wf-${x.id}-3`, 'plan-fill', '处置方案填写', '各金融机构', { 处置目标: '控制预警指标并逐步恢复至合理区间', 处置措施: x.plan, 责任部门: '机构风险管理部门', 责任人: '机构经办人', 计划完成时间: x.expectedReleaseDate || '持续推进' }, '提交处置方案', '已提交', x.attachments, x.logs[0]?.time));
      if (x.assessment) records.push(workflowRecord(`wf-${x.id}-4`, 'plan-review', '处置方案审核', '金控公司', { 审核意见: x.assessment }, '审核通过', '通过', [], x.logs[0]?.time));
      if (x.followUp) records.push(workflowRecord(`wf-${x.id}-6`, 'execution', '执行进展反馈', '各金融机构', { 本期执行情况: x.followUp, 下一步计划: '持续跟踪并按期反馈' }, '提交执行反馈', '持续跟踪', x.attachments, x.logs[0]?.time));
      const currentNodeId = x.status.includes('持续跟踪') ? 'execution' : x.status.includes('处置方案') ? 'plan-fill' : 'reason-fill';
      x.workflow = { currentNodeId, records };
    }
  });
  ...
  state.majorEvents.forEach(x => {
    ...
    if (!x.workflow) {
      const records = [workflowRecord(`wf-${x.id}-1`, 'event-create', '金融机构新建事件', '各金融机构', { 事件基本情况: x.basic, 初步分析研判: x.analysis, 已采取措施: x.measures }, '新建事件', '已完成', x.attachments, x.logs[0]?.time)];
      if (x.status !== '草稿') records.push(workflowRecord(`wf-${x.id}-2`, 'first-report', '提交首报及处置方案', '各金融机构', { 处置目标: x.target, 处置措施: x.plan, 责任部门: x.responsibleDept, 责任人: x.responsible }, '提交首报', '已提交', x.attachments, x.logs[0]?.time));
      if (/核实|处理中|归档|终报|续报/.test(`${x.currentStage}${x.status}`)) records.push(workflowRecord(`wf-${x.id}-3`, 'verify-report', '管理部门核实并组织汇报', '集团', { 核实意见: x.logs[0]?.content || '已完成核实' }, '核实', '通过', [], x.logs[0]?.time));
      x.followUps.forEach((report, index) => records.push(workflowRecord(`wf-${x.id}-6-${index}`, 'event-execution', '执行处置并续报', '各金融机构', { 最新进展: report.latestProgress, 风险变化: report.riskChange, 措施执行情况: report.execution, 下一步安排: report.nextStep }, `提交第${index + 1}次续报`, '持续跟踪', report.attachments, report.date)));
      if (x.finalReport) records.push(workflowRecord(`wf-${x.id}-7`, 'final-archive', '终报归档或常态跟踪', '各金融机构', { 最终处置结果: x.finalReport.result, 实际影响: x.finalReport.impact, 风险解除情况: x.finalReport.release, 后续安排: x.finalReport.followUp }, '提交终报', x.status === '已归档' ? '已归档' : '常态跟踪', x.finalReport.attachments, x.finalReport.date));
      x.workflow = { currentNodeId: x.status === '已归档' ? 'final-archive' : x.followUps.length ? 'event-execution' : x.status === '待审核' ? 'verify-report' : 'first-report', records };
    }
  });
