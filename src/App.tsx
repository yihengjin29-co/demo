...
function DashboardNew({ state, role, navigate, update, toast }: PageProps) {
  ...
  const yellow = state.warningDisposals.filter(item => item.level === '黄灯').length;
  const metrics = [
    ...
    { label: '专项风险事项', value: state.specialRisks.length, hint: '提示及工单', path: '/special-risks/manage' },
    { label: '历史指标数据', value: state.indicatorPeriodRecords.length, hint: '历史期次记录', path: '/reports' },
  ...
  return <Page title="并表管理驾驶舱" breadcrumb={['并表驾驶舱']}>
    <div className="cockpit-banner"><div><span className="cockpit-kicker">GROUP CONSOLIDATED RISK MANAGEMENT</span><h2>并表管理驾驶舱</h2><p>风险、指标、预警与重点事项运行态势</p></div><div className="cockpit-banner-actions"><Button variant="secondary" onClick={() => navigate('/workbench')}>进入工作台</Button><Button onClick={() => navigate('/warning/disposal')}>查看预警事项</Button></div></div>
    <div className="cockpit-kpis">{metrics.map(item => <button className="cockpit-kpi" key={item.label} onClick={() => navigate(item.path)}><span>{item.label}</span><b>{item.value}</b><small>{item.hint}　›</small></button>)}</div>
    <Section title="预警灯号概览" extra={<TextAction onClick={() => navigate('/warning/disposal')}>查看全部</TextAction>}><div className="lamp-grid"><button className="lamp-card red" onClick={() => navigate('/warning/disposal')}><i /><b>{red}</b><span>红灯预警</span><small>立即处置</small></button><button className="lamp-card yellow" onClick={() => navigate('/warning/disposal')}><i /><b>{yellow}</b><span>黄灯预警</span><small>持续跟踪</small></button><button className="lamp-card green" onClick={() => navigate('/warning/rules')}><i /><b>{Math.max(0, latest.length - red - yellow)}</b><span>绿灯</span><small>正常监测</small></button></div></Section>
    <Section title="重点指标监测" extra={canConfigureDashboard(role) ? <Button variant="secondary" onClick={() => { setDraftIds(selectedIds); setConfigOpen(true); }}>指标展示配置</Button> : undefined}>
      <div className="dashboard-indicator-grid">{selected.map(record => <button className="dashboard-indicator-card" key={record.id} onClick={() => navigate(`/reports/indicator/${record.id}`)}><div><span>{record.indicatorName}</span><StatusTag value={record.currentLightStatus} /></div><strong>{record.indicatorValue}</strong><p>{record.period} · {record.monitoringFrequency}</p><small>查看指标趋势及累计亮灯情况</small></button>)}</div>
    </Section>
    <div className="cockpit-two-grid"><Section title="重大风险事件概览" extra={<TextAction onClick={() => navigate('/major-events')}>进入台账</TextAction>}><div className="summary-strip"><button onClick={() => navigate('/major-events')}><b>{state.majorEvents.length}</b><span>事件总数</span></button><button onClick={() => navigate('/major-events')}><b>{state.majorEvents.filter(item => item.status === '处理中').length}</b><span>处理中</span></button><button onClick={() => navigate('/major-events')}><b>{state.majorEvents.filter(item => item.status === '已归档').length}</b><span>已归档</span></button></div></Section><Section title="专项风险提示概览" extra={<TextAction onClick={() => navigate('/special-risks/manage')}>查看详情</TextAction>}><div className="summary-strip"><button onClick={() => navigate('/special-risks/manage')}><b>{state.specialRisks.length}</b><span>提示总数</span></button><button onClick={() => navigate('/special-risks/feedback')}><b>{state.specialRisks.flatMap(item => item.workOrders).filter(item => item.status === '待反馈').length}</b><span>待反馈</span></button><button onClick={() => navigate('/special-risks/manage')}><b>{state.specialRisks.flatMap(item => item.workOrders).filter(item => item.status === '已解除').length}</b><span>已解除</span></button></div></Section></div>
...
function useRouter() {
  ...
  const replace = (next: string) => { window.history.replaceState({}, '', next); setPath(next); window.scrollTo(0, 0); };
...
function StatusTag({ value }: { value: string }) { const cls = /红灯|驳回|停用|已失效|已解除|归档/.test(value) ? 'red' : /黄灯|待审核|待反馈|待下发|评估中|草稿|待提交|暂停/.test(value) ? 'orange' : /绿灯|生效|已生效|已反馈|处理中|已报送|已下发|部分解除|提交/.test(value) ? 'green' : 'gray'; return <span className={`status ${cls}`}>{value}</span>; }
...
function Header({ role, setRole, onReset, onLogout }: { role: Role; setRole: (role: Role) => void; onReset: () => void; onLogout: () => void }) { return <header className="app-header"><div className="brand"><span className="brand-mark">▥</span><b>集团并表管理系统</b></div><div className="header-date">▣ 2024-06-30　13:22:45　 星期日</div><div className="header-right"><span className="bell">♧<i>12</i></span><span>?</span><span className="divider" /><span>{role === '各金融机构' ? currentInstitution : '集团并表范围'}⌄</span><label className="role-switch"><span>当前演示角色</span><select value={role} onChange={e => setRole(e.target.value as Role)}>{roleLabels.map(x => <option key={x}>{x}</option>)}</select></label><button className="reset-demo" onClick={onReset}>恢复演示数据</button><span className="avatar">张</span><span>张三</span><Button variant="text" onClick={onLogout}>退出登录</Button></div></header>; }
...
const menus: MenuItem[] = [
  ...
  { label: '预警管理', icon: '★', children: [{ label: '风险偏好及目标', path: '/warning/risk-preference', roles: ['集团', '金控公司'] }, { label: '风险预警规则管理', path: '/warning/rules' }, { label: '预警提示与处置', path: '/warning/disposal' }] },
  { label: '指标管理', icon: '▤', children: [{ label: '指标新增与维护', path: '/indicators/maintenance' }, { label: '指标版本管理', path: '/indicators/versions' }, { label: '最新期指标状态', path: '/indicators/latest-status' }] },
  { label: '重大风险事件管理', icon: '▧', children: [{ label: '重大风险事件列表', path: '/major-events' }, { label: '重大风险事件定义管理', path: '/major-events/definitions' }] },
...
function canAccessRouteForRole(role: Role, path: string) {
  ...
  if (/^\/special-risks\/feedback\/[^/]+\/evaluate$/.test(path)) return canHandleWorkflowNode(role, 'special-evaluate', currentInstitution);
...
function LatestIndicatorStatus({ state, role }: PageProps) {
  ...
  const [q, setQ] = useState({ code: '', name: '', type: '', subtype: '', institution: initialInstitution, frequency: '', light: '', period: '' });
  const latest = indicatorPeriodService.latest(state.indicatorPeriodRecords);
  const records = latest.filter(record => (!q.code || record.indicatorCode.includes(q.code)) && (!q.name || record.indicatorName.includes(q.name)) && (!q.type || record.indicatorType === q.type) && (!q.subtype || record.indicatorSubType.includes(q.subtype)) && (!q.institution || record.institution === q.institution) && (!q.frequency || record.monitoringFrequency === q.frequency) && (!q.light || record.currentLightStatus === q.light) && (!q.period || record.period.includes(q.period)));
...
function MajorEventDefinitions({ state, role, navigate, update, toast }: PageProps) {
  ...
  return <Page title="重大风险事件定义管理" breadcrumb={['重大风险事件管理', '重大风险事件定义管理']} actions={canManageMajorEventDefinitions(role) ? <Button onClick={() => navigate('/major-events/definitions/new')}>＋ 新增定义</Button> : undefined}><SearchPanel onSearch={() => undefined} onReset={() => setQ({ name: '', status: '', reference: '', updatedAt: '' })}><Field label="事件类型名称"><Input value={q.name} onChange={name => setQ({ ...q, name })} /></Field><Field label="定义状态"><Select value={q.status} onChange={status => setQ({ ...q, status })} options={['生效', '停用', '草稿']} /></Field><Field label="参考依据"><Input value={q.reference} onChange={reference => setQ({ ...q, reference })} /></Field><Field label="更新时间"><Input type="date" value={q.updatedAt} onChange={updatedAt => setQ({ ...q, updatedAt })} /></Field></SearchPanel><Table><thead><tr><th>序号</th><th>事件类型编码</th><th>事件类型名称</th><th>事件定义摘要</th><th>参考依据</th><th>定义状态</th><th>版本号</th><th>更新时间</th><th>操作</th><th>操作记录</th></tr></thead><tbody>{items.map((item, index) => <tr key={item.id}><td>{index + 1}</td><td>{item.code}</td><td>{item.name}</td><td className="ellipsis">{item.criteria.join(' ').slice(0, 100)}{item.criteria.join(' ').length > 100 ? '…' : ''}</td><td>{item.referenceBasis}</td><td><StatusTag value={item.status} /></td><td>{item.version}</td><td>{item.updatedAt}</td><td><TextAction onClick={() => navigate(`/major-events/definitions/${item.id}`)}>查看</TextAction>{canManageMajorEventDefinitions(role) && <TextAction onClick={() => navigate(`/major-events/definitions/${item.id}/edit`)}>编辑</TextAction>}{canManageMajorEventDefinitions(role) && item.status !== '生效' && <TextAction onClick={() => setStatus(item, '生效')}>启用</TextAction>}{canManageMajorEventDefinitions(role) && item.status === '生效' && <TextAction onClick={() => setStatus(item, '停用')}>停用</TextAction>}</td><td><TextAction onClick={() => toast(item.logs.map(log => `${log.time} ${log.action} ${log.content}`).join('\n'))}>查看操作记录</TextAction></td></tr>)}</tbody></Table><Pagination total={items.length} page={1} setPage={() => undefined} /></Page>;
}
...
function MajorEventDefinitionEditor({ state, role, navigate, update, toast }: PageProps) {
  ...
  if (!canManageMajorEventDefinitions(role)) return <Page title="重大风险事件定义管理" breadcrumb={['重大风险事件管理']} actions={<Button variant="secondary" onClick={() => navigate('/major-events/definitions')}>返回</Button>}><div className="empty-state"><h2>当前角色无权维护事件定义</h2></div></Page>;
...
function Reports({ state, role, navigate }: PageProps) {
  const initialInstitution = role === '各金融机构' ? currentInstitution : '';
  const [historyItem, setHistoryItem] = useState<IndicatorPeriodRecord | null>(null);
  ...
  const records = indicatorPeriodService.history(state.indicatorPeriodRecords).filter(record => (!q.code || record.indicatorCode.includes(q.code)) && (!q.name || record.indicatorName.includes(q.name)) && (!q.type || record.indicatorType === q.type) && (!q.subtype || record.indicatorSubType.includes(q.subtype)) && (!q.institution || record.institution === q.institution) && (!q.frequency || record.monitoringFrequency === q.frequency) && (!q.light || record.currentLightStatus === q.light) && (!startOrder || record.periodOrder >= startOrder) && (!endOrder || record.periodOrder <= endOrder));
  const summaryText = (record: IndicatorPeriodRecord) => { const summary = calculateLightSummaryForRecords(getIndicatorHistoryRecords(state.indicatorPeriodRecords, record)); return `累计红灯${summary.red}次，黄灯${summary.yellow}次，绿灯${summary.green}次`; };
  const exportRows = () => downloadCSV('历史指标数据.csv', [['指标编码', '指标名称', '监测频率', '指标值', '当期亮灯情况', '累计亮灯情况', '指标定义', '指标类型', '指标子类', '适用机构', '预测范围', '黄灯规则', '红灯规则', '指标期次'], ...records.map(record => [record.indicatorCode, record.indicatorName, record.monitoringFrequency, record.indicatorValue, record.currentLightStatus, summaryText(record), record.indicatorDefinition, record.indicatorType, record.indicatorSubType, record.institution, record.forecastRange, record.yellowRule, record.redRule, record.period])]);
  return <Page title="报表中心" breadcrumb={['报表中心', '历史指标数据查询']}><SearchPanel onSearch={() => undefined} onReset={() => setQ({ code: '', name: '', type: '', subtype: '', institution: initialInstitution, frequency: '', light: '', periodStart: '', periodEnd: '' })}><Field label="指标编码"><Input value={q.code} onChange={code => setQ({ ...q, code })} /></Field><Field label="指标名称"><Input value={q.name} onChange={name => setQ({ ...q, name })} /></Field><Field label="指标类型"><Select value={q.type} onChange={type => setQ({ ...q, type })} options={['财务类', '资本类', '风险类']} /></Field><Field label="指标子类"><Input value={q.subtype} onChange={subtype => setQ({ ...q, subtype })} /></Field><Field label="适用机构"><Select disabled={role === '各金融机构'} value={q.institution} onChange={institution => setQ({ ...q, institution })} options={role === '各金融机构' ? [currentInstitution] : institutions} /></Field><Field label="监测频率"><Select value={q.frequency} onChange={frequency => setQ({ ...q, frequency })} options={['日', '周', '月', '季', '半年', '年', '不定期']} /></Field><Field label="当期亮灯"><Select value={q.light} onChange={light => setQ({ ...q, light })} options={['绿灯', '黄灯', '红灯']} /></Field><Field label="指标期次"><div className="date-range"><Input value={q.periodStart} onChange={periodStart => setQ({ ...q, periodStart })} placeholder="起始期次" /><span>至</span><Input value={q.periodEnd} onChange={periodEnd => setQ({ ...q, periodEnd })} placeholder="结束期次" /></div></Field></SearchPanel><div className="list-toolbar"><span>历史指标数据 · {records.length} 条</span><Button variant="secondary" onClick={exportRows}>⇩ 导出</Button></div><IndicatorPeriodTable records={records} onViewCumulative={setHistoryItem} /><Pagination total={records.length} page={1} setPage={() => undefined} />{historyItem && <IndicatorLightHistoryModal record={historyItem} records={state.indicatorPeriodRecords} onClose={() => setHistoryItem(null)} />}</Page>;
}
...
function SpecialRiskView({ state, role, navigate }: PageProps) {
  ...
  const order = item?.workOrders.find(workOrder => workOrder.id === orderId) || item?.workOrders[0];
  const workflow = order?.workflow || { currentNodeId: 'special-feedback', records: [] };
  const [selectedNodeId, setSelectedNodeId] = useState(workflow.currentNodeId);
  if (!item) return <Page title="专项风险提示详情" breadcrumb={['专项风险管理']} actions={<Button variant="secondary" onClick={() => navigate('/special-risks/manage')}>返回</Button>}><div className="empty-state"><h2>无权查看该专项风险事项</h2></div></Page>;
  return <Page title="专项风险提示与工单详情" breadcrumb={['专项风险管理', '专项风险提示与管理', item.name]} actions={<><Button variant="secondary" onClick={() => navigate('/special-risks/manage')}>返回</Button><Button variant="secondary" onClick={() => downloadText(item.name + '.txt', item.purpose + '\n' + item.measures + '\n' + item.requirements)}>附件下载</Button></>}>
    <Section title="提示基本信息"><div className="detail-grid"><b>专项风险提示名称<span>{item.name}</span></b><b>专项风险类型<span>{item.types.join('、')}</span></b><b>下发机构<span>{item.institutions.join('、')}</span></b><b>创建时间<span>{item.createdAt}</span></b><b>当前工单<span>{order?.id || '—'}</span></b><b>工单机构<span>{order?.institution || '—'}</span></b><b>当前状态<span>{order ? <StatusTag value={order.status} /> : '—'}</span></b><b>当前节点<span>{specialRiskWorkflow.find(node => node.id === workflow.currentNodeId)?.name || '—'}</span></b></div><div className="read-block"><b>提示背景与目的</b><p>{item.purpose}</p><b>管理措施与建议</b><p>{item.measures}</p><b>自检与反馈要求</b><p>{item.requirements}</p></div></Section>
    {order && <><InteractiveWorkflow definition={specialRiskWorkflow} instance={workflow} selectedNodeId={selectedNodeId} onSelect={setSelectedNodeId} /><WorkflowFeedbackOverview definition={specialRiskWorkflow} instance={workflow} role={role} institution={order.institution} selectedNodeId={selectedNodeId} onReturnCurrent={() => setSelectedNodeId(workflow.currentNodeId)} />{selectedNodeId === workflow.currentNodeId && canHandleWorkflowNode(role, workflow.currentNodeId, order.institution) && <Section title="当前节点操作"><div className="form-actions">{['special-feedback', 'special-track'].includes(workflow.currentNodeId) ? <Button onClick={() => navigate(`/special-risks/feedback/${item.id}?order=${order.id}`)}>进入反馈填写</Button> : <Button onClick={() => navigate(`/special-risks/feedback/${item.id}/evaluate?order=${order.id}`)}>进入评估办理</Button>}</div></Section>}</>}
    <Section title="全部机构工单"><Table><thead><tr><th>机构</th><th>工单编号</th><th>反馈时间</th><th>联系人</th><th>状态</th><th>评估结果</th><th>操作</th></tr></thead><tbody>{item.workOrders.map(workOrder => <tr key={workOrder.id}><td>{workOrder.institution}</td><td>{workOrder.id}</td><td>{workOrder.feedback?.date || '—'}</td><td>{workOrder.feedback?.contact || '—'}</td><td><button className="status-link" onClick={() => navigate('/special-risks/manage/' + item.id + '?order=' + workOrder.id)}><StatusTag value={workOrder.status} /></button></td><td>{workOrder.evaluation?.result || '—'}</td><td><TextAction onClick={() => navigate('/special-risks/manage/' + item.id + '?order=' + workOrder.id)}>查看流程</TextAction></td></tr>)}</tbody></Table></Section>
  </Page>;
}
...
function SpecialRiskFeedbackList({ state, role, navigate, update, toast }: PageProps) { const [q, setQ] = useState({ id: '', type: '', status: '' }); const rows = state.specialRisks.flatMap(item => item.workOrders.map(order => ({ item, order }))).filter(x => (!q.id || x.order.id.includes(q.id)) && (!q.type || x.item.types.includes(q.type)) && (!q.status || x.order.status === q.status)); return <Page title="专项风险查看与反馈" breadcrumb={['专项风险管理', '专项风险查看与反馈']}><SearchPanel onSearch={() => undefined} onReset={() => setQ({ id: '', type: '', status: '' })}><Field label="工单编号"><Input value={q.id} onChange={v => setQ({ ...q, id: v })} /></Field><Field label="下发起止时间"><div className="date-range"><Input type="date" /><span>至</span><Input type="date" /></div></Field><Field label="专项风险类型"><Select value={q.type} onChange={v => setQ({ ...q, type: v })} options={specialTypes} /></Field><Field label="状态"><Select value={q.status} onChange={v => setQ({ ...q, status: v })} options={['待反馈', '已反馈', '评估中', '部分解除', '已解除', '待跟踪反馈']} /></Field></SearchPanel><Table><thead><tr><th>关联工单编号</th><th>下发日期</th><th>上次反馈时间</th><th>专项风险类型</th><th>提示名称</th><th>查看提示</th><th>状态</th><th>反馈</th><th>解除日期</th><th>报告查看</th></tr></thead><tbody>{rows.map(({ item, order }) => <tr key={order.id}><td>{order.id}</td><td>{order.issuedAt}</td><td>{order.feedback?.date || '—'}</td><td>{item.types.join('、')}</td><td>{item.name}</td><td><TextAction onClick={() => navigate(`/special-risks/manage/${item.id}`)}>查看</TextAction></td><td><button className="status-link" onClick={() => navigate(`/special-risks/manage/${item.id}?order=${order.id}`)}><StatusTag value={order.status} /></button></td><td>{order.status === '待反馈' || order.status === '待跟踪反馈' ? <TextAction onClick={() => navigate(`/special-risks/feedback/${item.id}?order=${order.id}`)}>{order.status === '待跟踪反馈' ? '跟踪反馈填写' : '反馈填写'}</TextAction> : '已反馈'}</td><td>{order.evaluation?.result === '解除' ? order.evaluation.date : '—'}</td><td>{order.feedback ? <TextAction onClick={() => navigate(`/special-risks/feedback/${item.id}/evaluate?order=${order.id}`)}>查看管理报告</TextAction> : '—'}</td></tr>)}</tbody></Table><Pagination total={rows.length} page={1} setPage={() => undefined} /></Page>; }

function SpecialRiskFeedback({ state, role, navigate, update, toast }: PageProps) { const id = window.location.pathname.split('/')[3]; const item = state.specialRisks.find(x => x.id === id) || state.specialRisks[0]; const orderId = new URLSearchParams(window.location.search).get('order') || undefined; const order = getOrder(item, orderId); const [form, setForm] = useState({ contact: order.feedback?.contact || '', phone: order.feedback?.phone || '', understanding: order.feedback?.understanding || '', problems: order.feedback?.problems || '', strategy: order.feedback?.strategy || '', other: order.feedback?.other || '' }); const [files, setFiles] = useState<Attachment[]>(order.feedback?.attachments || []); const workflowNodeId = order.workflow?.currentNodeId || 'special-feedback'; if (!canHandleWorkflowNode(role, workflowNodeId, order.institution)) return <Page title="专项风险反馈填写" breadcrumb={['专项风险管理']} actions={<Button variant="secondary" onClick={() => navigate('/special-risks/feedback')}>返回</Button>}><div className="empty-state"><h2>当前角色无权办理该节点</h2></div></Page>; const save = () => { if (!form.contact || !form.understanding || !form.problems || !form.strategy) return toast('请填写联系人、认识理解、自查问题和风险应对策略'); update(s => { const target = s.specialRisks.find(x => x.id === item.id)!; const targetOrder = target.workOrders.find(x => x.id === order.id); if (!targetOrder) return; targetOrder.feedback = { ...form, attachments: files, date: today() }; targetOrder.workflow ||= { currentNodeId: 'special-feedback', records: [] }; const node = specialRiskWorkflow.find(candidate => candidate.id === targetOrder.workflow!.currentNodeId) || specialRiskWorkflow[1]; workflowService.append(targetOrder.workflow, node, { role, action: node.id === 'special-track' ? '提交跟踪反馈' : '提交首次反馈', result: '已反馈', formData: { 联系人: form.contact, 联系电话: form.phone, 认识与理解: form.understanding, 自查问题与风险点: form.problems, 风险应对策略: form.strategy, 其他反馈: form.other }, attachments: files }); workflowService.moveNext(targetOrder.workflow, specialRiskWorkflow, node.id); targetOrder.status = '评估中'; target.logs.push(createLog('反馈提交', `${targetOrder.institution} 提交专项风险反馈`, role)); }); toast('机构反馈已保存，状态更新为评估中'); navigate('/special-risks/feedback'); }; return <Page title="专项风险反馈填写" breadcrumb={['专项风险管理', '专项风险查看与反馈', '反馈填写']} actions={<><Button variant="secondary" onClick={() => navigate('/special-risks/feedback')}>关闭</Button><Button onClick={save}>保存</Button></>}><Section title="专项风险提示基本信息"><div className="detail-grid"><b>提示名称<span>{item.name}</span></b><b>专项风险类型<span>{item.types.join('、')}</span></b><b>下发机构<span>{order.institution}</span></b></div><div className="read-block"><b>提示背景与目的</b><p>{item.purpose}</p><b>管理措施与建议</b><p>{item.measures}</p><b>自检与反馈要求</b><p>{item.requirements}</p></div></Section><Section title="机构反馈"><div className="form-grid"><Field label="反馈机构"><Input value={order.institution} disabled /></Field><Field label="联系人" required><Input value={form.contact} onChange={v => setForm({ ...form, contact: v })} /></Field><Field label="联系电话"><Input value={form.phone} onChange={v => setForm({ ...form, phone: v })} /></Field><Field label="对专项风险提示的认识与理解" required><Textarea value={form.understanding} onChange={v => setForm({ ...form, understanding: v })} /></Field><Field label="自查发现的问题与风险点" required><Textarea value={form.problems} onChange={v => setForm({ ...form, problems: v })} /></Field><Field label="未来风险预防与应对策略" required><Textarea value={form.strategy} onChange={v => setForm({ ...form, strategy: v })} /></Field><Field label="其他总结反馈"><Textarea value={form.other} onChange={v => setForm({ ...form, other: v })} /></Field></div></Section><Section title="附件"><FileUploader files={files} onChange={setFiles} /></Section></Page>; }

function SpecialRiskEvaluate({ state, role, navigate, update, toast }: PageProps) { const id = window.location.pathname.split('/')[3]; const item = state.specialRisks.find(x => x.id === id) || state.specialRisks[0]; const orderId = new URLSearchParams(window.location.search).get('order') || undefined; const order = getOrder(item, orderId); const [form, setForm] = useState({ result: order.evaluation?.result || '部分解除', department: order.evaluation?.department || '金融机构管理部', summary: order.evaluation?.summary || '' }); const [files, setFiles] = useState<Attachment[]>(order.evaluation?.attachments || []); const workflowNodeId = order.workflow?.currentNodeId || 'special-evaluate'; if (!canHandleWorkflowNode(role, workflowNodeId, order.institution)) return <Page title="专项风险反馈评估" breadcrumb={['专项风险管理']} actions={<Button variant="secondary" onClick={() => navigate('/special-risks/manage')}>返回</Button>}><div className="empty-state"><h2>当前角色无权办理该节点</h2></div></Page>; const save = (draft: boolean) => { if (!draft && !form.summary) return toast('请填写分析及总结'); update(s => { const target = s.specialRisks.find(x => x.id === item.id)!; const targetOrder = target.workOrders.find(x => x.id === order.id); if (!targetOrder) return; targetOrder.evaluation = { ...form, result: form.result as '解除' | '部分解除' | '驳回', date: today(), attachments: files }; targetOrder.workflow ||= { currentNodeId: 'special-evaluate', records: [] }; const node = specialRiskWorkflow.find(candidate => candidate.id === targetOrder.workflow!.currentNodeId) || specialRiskWorkflow[2]; workflowService.append(targetOrder.workflow, node, { role, action: draft ? '保存评估草稿' : '提交管理评估', result: form.result, formData: { 评估结果: form.result, 评估部门: form.department, 分析及总结: form.summary }, opinion: form.summary, returnReason: form.result === '驳回' ? form.summary : undefined, attachments: files, status: form.result === '驳回' ? '已退回' : undefined }); if (!draft) { if (form.result === '驳回') workflowService.returnTo(targetOrder.workflow, 'special-track'); else workflowService.moveNext(targetOrder.workflow, specialRiskWorkflow, node.id); } targetOrder.status = form.result === '解除' ? '已解除' : form.result === '部分解除' ? '部分解除' : '待跟踪反馈'; target.logs.push(createLog(draft ? '保存评估草稿' : '提交评估', `${targetOrder.institution} 评估结果：${form.result}`, role)); }); toast(draft ? '评估草稿已保存' : '评估结果已保存并同步机构工单状态'); if (!draft) navigate('/special-risks/manage'); }; return <Page title="专项风险反馈评估" breadcrumb={['专项风险管理', '专项风险查看与反馈', '反馈评估']} actions={<><Button variant="secondary" onClick={() => navigate('/special-risks/manage')}>关闭</Button><Button variant="secondary" onClick={() => save(true)}>保存草稿</Button><Button onClick={() => save(false)}>保存</Button></>}><Section title="提示基本信息"><div className="detail-grid"><b>提示名称<span>{item.name}</span></b><b>类型<span>{item.types.join('、')}</span></b><b>反馈机构<span>{order.institution}</span></b></div></Section><Section title="机构反馈信息"><div className="read-block"><b>联系人及电话</b><p>{order.feedback?.contact || '—'} {order.feedback?.phone || ''}</p><b>认识与理解</b><p>{order.feedback?.understanding || '—'}</p><b>自查问题与风险点</b><p>{order.feedback?.problems || '—'}</p><b>未来风险预防与应对策略</b><p>{order.feedback?.strategy || '—'}</p><b>其他总结反馈</b><p>{order.feedback?.other || '—'}</p></div></Section><Section title="评估区"><div className="form-grid"><Field label="评估结果" required><Select value={form.result} onChange={v => setForm({ ...form, result: v })} options={['解除', '部分解除', '驳回']} /></Field><Field label="评估部门" required><Select value={form.department} onChange={v => setForm({ ...form, department: v })} options={departments} /></Field><Field label="分析及总结" span={2}><Textarea value={form.summary} onChange={v => setForm({ ...form, summary: v })} /></Field></div></Section><Section title="反馈附件"><FileUploader files={files} onChange={setFiles} /></Section></Page>; }
