...
function RoleWorkbench({ state, role, navigate }: { state: DemoState; role: Role; navigate: (path: string) => void }) {
  ...
  return <Page title="工作台" breadcrumb={['工作台']}>
    <div className="workbench-head"><div><span className="cockpit-kicker">ROLE-BASED WORKSPACE</span><h2>{data.title}</h2><p>{data.description}</p></div><div className="workbench-head-actions"><Button variant="secondary" onClick={() => navigate('/concentration-monitoring')}>集中度风险监测</Button><Button onClick={() => navigate('/dashboard')}>返回驾驶舱</Button></div></div>
    <div className="workbench-kpis role-workbench-kpis">{data.statistics.map(item => <button key={item.label} onClick={() => navigate(item.path)}><span>{item.label}</span><b>{item.value}</b><small>{item.note}</small></button>)}</div>
    <div className="workbench-main-grid role-workbench-main">
      <Section title={data.mode === 'management' ? '管理待办' : '本机构待办'} extra={<Tabs items={categories} active={category} onChange={setCategory} />}>
        <Table><thead><tr><th>任务事项</th><th>任务类型</th><th>所属机构</th><th>办理时限</th><th>状态</th><th>操作</th></tr></thead><tbody>{tasks.map(item => <tr key={item.id}><td>{item.title}</td><td>{item.category}</td><td>{item.institution}</td><td>{item.deadline}</td><td><StatusTag value={item.status} /></td><td><TextAction onClick={() => navigate(item.path)}>{item.action}</TextAction></td></tr>)}</tbody></Table>
        {!tasks.length && <div className="empty-state compact"><p>当前角色暂无此类待办事项</p></div>}
      </Section>
      <Section title="重点关注"><div className="workbench-focus-list">{data.focus.map(item => <button key={item.id} onClick={() => navigate(item.path)}><div><b>{item.title}</b><span>{item.detail}</span></div><StatusTag value={item.status} /></button>)}{!data.focus.length && <div className="empty-state compact"><p>暂无重点关注事项</p></div>}</div></Section>
    </div>
    {data.mode === 'management' ? <Section title="机构风险概览"><Table><thead><tr><th>机构名称</th><th>红灯预警</th><th>黄灯预警</th><th>重大风险事件</th><th>待处理事项</th><th>数据报送状态</th><th>操作</th></tr></thead><tbody>{data.institutions.map(item => <tr key={item.institution}><td>{item.institution}</td><td><b className="risk-number red">{item.red}</b></td><td><b className="risk-number yellow">{item.yellow}</b></td><td>{item.majorEvents}</td><td>{item.pending}</td><td><StatusTag value={item.reportStatus} /></td><td><TextAction onClick={() => navigate(`/institutions/${encodeURIComponent(item.institution)}`)}>查看机构</TextAction></td></tr>)}</tbody></Table></Section> : <Section title="本机构风险概览"><div className="workbench-concentration-summary">{data.concentrationSummary.map(item => <button key={item.type} onClick={() => navigate('/concentration-monitoring')}><b>{item.type}</b><span className="red">红灯 {item.red}</span><span className="yellow">黄灯 {item.yellow}</span><span className="green">绿灯 {item.green}</span></button>)}</div></Section>}
    <Section title="最近办理记录"><Table><thead><tr><th>操作事项</th><th>办理说明</th><th>办理时间</th></tr></thead><tbody>{data.recent.map(item => <tr key={item.id}><td>{item.action}</td><td>{item.detail}</td><td>{item.time}</td></tr>)}</tbody></Table></Section>
  </Page>;
}
